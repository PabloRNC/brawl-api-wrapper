...
export function getSeasonReset(
	brawlers: Brawler[],
	totalTrophies: number
): { remainingTrophies: number; trophyBox: TrophyBox } {
	const reducedTrophies = brawlers.reduce(
		(acc, x) => acc + (x.trophies > 1000 ? x.trophies - 1000 : 0),
		0
	)
