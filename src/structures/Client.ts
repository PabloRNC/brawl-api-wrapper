...
export class Client {
  ...
  public async getBrawler(id: number): Promise<Brawler> {
    return new Brawler(await new Request(this.token).getBrawler(id));
  }
  public async getBrawlers(): Promise<Brawler[]> {
    return await new Request(this.token).getBrawlers();
  }
  public async getRankingOfPlayers(countryCode: string = "global") : Promise<RankingOfPlayersResponse[]>{
    return await new Request(this.token).getRankingOfPlayers(countryCode)
  }
  public async getRankingOfClubs(countryCode: string = "global"): Promise<RankingOfClubsResponse[]>{
    return await new Request(this.token).getRankingOfClubs(countryCode)
  }
  public async getRankingOfBrawlers(brawler: Brawlers, countryCode: string = "global"): Promise<RankingOfPlayersResponse[]>{
