...
import { Brawler, Gadget, Gear, StarPower } from "../types/Brawler"
export class Player{
    public tag: string
    public name: string
    ...
    public getBrawler(name: string): Brawler{
        return this.brawlers.find(brawler => brawler.name.toLowerCase() === name.toLowerCase())
    }
    public getGadget(name: string): Gadget{
        return this.brawlers.find((brawler) => brawler.gadgets.some(gadget => gadget.name.toLowerCase() === name.toLowerCase())).gadgets.find((gadget) => gadget.name.toLowerCase() === name.toLowerCase())
    }
    public getStarPower(name: string): StarPower{
        return this.brawlers.find((brawler) => brawler.starPowers.some(starPower => starPower.name.toLowerCase() === name.toLowerCase())).starPowers.find((starPower) => starPower.name.toLowerCase() === name.toLowerCase())
    }
    public getGear(name: string): Gear{
        return this.brawlers.find((brawler) => brawler.gears.some((gear) => gear.name.toLowerCase() === name.toLowerCase())).gears.find((gear) => gear.name.toLowerCase() === name.toLowerCase())
    }
    public sortBrawlers(filter: "TROPHIES" | "POWER" | "RANK", criteria: "ASCENDING" | "DESCENDING"): Brawler[] {
        switch(filter){
            case "TROPHIES": {
                return criteria === "ASCENDING" ? this.brawlers.sort((a, b) => b.trophies - a.trophies)
                : this.brawlers.sort((a, b) => a.trophies - b.trophies)
            }
            break;
            case "POWER" : {
                return criteria === "ASCENDING" ? this.brawlers.sort((a, b) => b.power - a.power)
                : this.brawlers.sort((a, b) => a.power - b.power)
            }
            break;
            case "RANK" : {
                return criteria === "ASCENDING" ? this.brawlers.sort((a, b) => b.rank - a.rank)
